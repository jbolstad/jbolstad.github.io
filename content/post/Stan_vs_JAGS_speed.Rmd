...
jags <- data.frame(jags)
stan <- data.frame(stan)
names(jags) <- c("alpha[1]","beta[1]")
names(stan) <- c("alpha[1]","beta[1]")

p <- ggplot(plotdat, aes(`alpha[1]`, `beta[1]`)) + geom_point(color=col.trans[2,8], alpha=.2) + facet_wrap(sampler ~ .) 
p 
...
p = plot_grid(
  ggmatrix_gtable(ggpairs(jags)),
  ggmatrix_gtable(ggpairs(stan)),
  nrow = 2
)
```
